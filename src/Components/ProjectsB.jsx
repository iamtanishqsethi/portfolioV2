...
const ProjectsB=({data})=>{
    ...
    return(
        <div
            ref={ref}
            className={`group transition-all duration-1000 ${isVisible?'opacity-100 translate-x-0':'opacity-0'} ${ data.id%2===0?' -translate-x-10 ':' translate-x-30 '} relative p-4 m-8 md:my-28 lg:mb-0`}>
            <div className={`   transition ease-in-out bg-black md:absolute  my-2 md:my-0 w-full md:w-1/2  ${data.id%2===0?"md:right-1/2 md:top-1/3 text-center md:text-left  md:group-hover:translate-x-2":"md:left-1/2 md:top-1/3 text-center md:text-right  md:group-hover:-translate-x-2"}  py-8 px-6 rounded-xl bg-opacity-80`}>
                <h1
